...
public class ForkJoinFramework {
  ...
  // =========================================================================================================================================
  // RecursiveAction
  static void testRecursiveAction() throws Exception {
    ...
    RandomIntRecursiveAction action = new RandomIntRecursiveAction(data, 0, data.length,
        "Action " + ACTION_COUNT.getAndIncrement());
  ...
  public static class RandomIntRecursiveAction extends RecursiveAction {
    ...
    public RandomIntRecursiveAction(int[] data, int start, int end, String name) {
      super();
      this.data = data;
    ...
    protected void compute() {
      ...
      if (end - start <= THRESHOLD) { // is it a manageable amount of work?
      ...
      } else { // task to big, split it
        int halfWay = ((end - start) / 2) + start;

        RandomIntRecursiveAction leftRecursive = new RandomIntRecursiveAction(data, start,
            halfWay, "Left: " + ACTION_COUNT.getAndIncrement());
        // leftRecursive.fork(); // 1º queue left half of task, will be execute on parallel

        RandomIntRecursiveAction rightRecursive = new RandomIntRecursiveAction(data, halfWay, end,
            "Right: " + ACTION_COUNT.getAndIncrement());
        ...
        ForkJoinTask
            .invokeAll(leftRecursive, rightRecursive); // shortcut for compute, fork() & join()
  ...
  // =========================================================================================================================================
  // RecursiveTask
  static void testRecursiveTask() throws Exception {

    int[] data = new int[10_000_000];

    ForkJoinPool forkJoinPool = new ForkJoinPool();

    forkJoinPool.invoke(new RandomIntRecursiveAction(data, 0, data.length,
        "Action " + ACTION_COUNT.getAndIncrement())); // fill the array
    ...
    final Integer position = forkJoinPool.invoke(task);
  ...
  static class FindMaxPositionRecursiveTask extends RecursiveTask<Integer> {
    ...
    protected Integer compute() { // return type matches the <generic> type
      if (end - start <= THRESHOLD) { // is it a manageable amount of work?
        ...
        FindMaxPositionRecursiveTask leftRecursive = new FindMaxPositionRecursiveTask(data, start,
            halfWay);
        leftRecursive.fork(); // queue left half of task, will be execute on parallel

        FindMaxPositionRecursiveTask rightRecursive = new FindMaxPositionRecursiveTask(data,
            halfWay, end);
        int position2 = rightRecursive
            .compute(); // work ro right half of task, put the rightRecursive on the stack

        int position1 = leftRecursive
            .join(); // wait for queued task to be complete, we need de result
   ...
   *
   * 1. First check whether the task is small enough to be performed directly without forking.
   * If so, perform it without forking.
   *
   * 2. If no, then split the task into multiple small tasks (at least 2) and submit the subtasks back to
   * the pool using invokeAll(list of tasks) or leftRecursive.fork(), rightRecursive.compute(), leftRecursive.join()
   */
